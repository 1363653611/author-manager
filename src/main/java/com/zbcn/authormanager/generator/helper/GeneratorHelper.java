package com.zbcn.authormanager.generator.helper;
...
import com.zbcn.authormanager.generator.entity.GeneratorConstant;
 ...
 * @author zbcn8
 * @version 1.0.0
 * @ClassName GeneratorHelper.java
 * @Description 生成器
 * @createTime 2019年08月03日 10:46:00
 */
@Slf4j
@Helper
public class GeneratorHelper {
    ...
    public void generateEntityFile(List<Column> columns, GeneratorConfig configure) throws Exception {
        ...
        columns.forEach(c -> {
        c.setField(AuthorUtils.underscoreToCamel(StringUtils.lowerCase(c.getName())));
    ...
    public void generateMapperXmlFile(List<Column> columns, GeneratorConfig configure) throws Exception {
        ...
        JSONObject data = toJSONObject(configure);
        columns.forEach(c -> c.setField(AuthorUtils.underscoreToCamel(StringUtils.lowerCase(c.getName()))));
        data.put("columns", columns);
